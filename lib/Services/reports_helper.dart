...
class reports_helper {
  ...
  {
    HttpsCallable getter = FirebaseFunctions.instance.httpsCallable(
        'getZReport');
  ...
  Future<Map<String, double>> get_all_z_reports() async
  {
    ...
    for (int i = 0; i < data.length; ++i) {
      ...
      double amount = double.parse(money.substring(1, money.length));
      z_reports.update(data[i]['date_field_string'], (value) => amount,
          ifAbsent: () => amount);
  ...
  Future<void> update_x_report(double amount) async
  {
    ...
    if (new_amount - amount == 0) {
      HttpsCallable updater = FirebaseFunctions.instance.httpsCallable(
          'makeZReport');
  ...
  Future<List<sales_report_row>> generate_sales_report(String date1,
      String date2) async
  {
    Map<int, int> sales = {};
    List<sales_report_row> report = [];
    HttpsCallable get_sales = FirebaseFunctions.instance.httpsCallable(
        'getItemsInOrder');
    final res = await get_sales.call({
      'date1': date1,
      'date2': date2
    });
    ...
    for (int index = 0; index < data.length; ++index) {
      ...
      l.sort();
      for (int i in l) {
        sales.update(i, (value) => value + 1, ifAbsent: () => 1);
      }
    }

    HttpsCallable get_price = FirebaseFunctions.instance.httpsCallable('getItemPrice');
    for (MapEntry<int, int> e in sales.entries) {
      String type = await gen_helper.get_item_type(e.key);
      int id = e.key;
      String item_name = await gen_helper.get_item_name(e.key);
      int amount_sold = e.value;
      final res = await get_price.call({'menu_item_name': item_name});
      List<dynamic> data = res.data;
      double price = data[0]['item_price'];
      double total_revenue = price * amount_sold;
